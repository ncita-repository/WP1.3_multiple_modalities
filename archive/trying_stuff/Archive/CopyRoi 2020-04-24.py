...
def CopyRoi(XnatAddress, XnatUsername, XnatPassword, RootDir, CopyFrom, CopyTo, \
    ...
    if Shift:
        ...
        N = len(DataDict['Source']['ScanPos'])
        
        print(f'\nThe source scan positions are (N = {N}):\n\n', \
              DataDict['Source']['ScanPos'])
            
        print(f'\nThe pre-shifted scan positions were (N = {N}):\n\n', \
              DataDict['Target']['ScanPos'])
        
        print(f'\nThe post-shifted scan positions are (N = {N}):\n\n', \
    ...
    """ APRIL 24: See below for new (hopefully simpler) approach... """    
    ...
    """ APRIL 24: New (hopefully simpler) approach to remapping which also 
    accounts for the fact that with the new approach taken to omit slices that
    have large positional differenes, both Source and Target DICOMs will need
    to be remapped, not only the series that had a larger number of slices. """
