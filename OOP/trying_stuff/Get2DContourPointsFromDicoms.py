# -*- coding: utf-8 -*-
"""
Created on Thu Jun  4 11:57:47 2020
...
"""
Function:
    Get2DContourPointsFromDicoms()
    
Purpose:
    Get array of 2D contour points from an ROI object that correspond to a
    series of DICOM objects contained in a directory, e.g.:
        ...
        where each sub-array contains the [x, y] points contained in a 
...
Input:
    ...
    Flattened     - (Boolean) If True, each row in ContourPoints is a point
                    If False, each row contains an array of points for each
                    DICOM (or an empty array [] if there are no points for
                    that DICOM)
...
def Get2DContourPointsFromDicoms(DicomDir, RoiFpath, Flattened):
    # Import packages:
    import pydicom
    ...
    for Dicom in Dicoms:
        ...
        if SopUid in RefSopUids:
            ...
            for ind in inds:
                ...
                #if Flattened:
                #    # Only append if ContourPoints_c is not []:
                #    if ContourPoints_c:
                #        ContourPoints.append(ContourPoints_c)
                #else:
                #    # Append contourPts with ContourPts_c:
        ...
        if Flattened:
            # Only append if ContourPoints is not []:
            if ContourPoints: 
                # Append ContourPoints to AllContourPts:
                AllContourPoints.extend(ContourPoints)
        else:
            # Append ContourPoints to AllContourPts:
            AllContourPoints.append(ContourPoints)
