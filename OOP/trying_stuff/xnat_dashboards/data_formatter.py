...
class FormatterPP(Formatter):
    ...
    def get_projects_details(self, projects):
        ...
        # Add PI and co-investigators (CT):
        project_details['PI'] = project_dict['pi']
        
        if project_details['PI'] == '':
            project_details['PI'] = ['------']
        
        project_details['Co-investigator(s)']\
            = project_dict['project_invs'].split('<br/>')
        
        if project_details['Co-investigator(s)'][0] == '':
            project_details['Co-investigator(s)'] = ['------']
