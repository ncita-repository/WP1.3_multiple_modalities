...
class Fetcher:
    ...
    def get_instance_details(self):
        ...
        try:
            ...
            # Try gathering all data as for projects:
            ##subjects = self.selector.get('/data/subjects', 
            ##                             params={'columns': 'ID,project,handedness,age,gender'})\
            ##    .json()['ResultSet']['Result']
            #subjects = self.selector.get('/data/subjects').json()['ResultSet']['Result']
            
            """ That fetched only 'insert_date', 'project', 'ID', 'insert_user',
            and 'URI', and doesn't have 'age', 'gender', handedness...not what
            I was expecting. 
            
            session.select('xnat:subjectData').all().data
            
            returns insert_date, insert_user, gender_text, handedness_text, dob,
            ... project, ... 
