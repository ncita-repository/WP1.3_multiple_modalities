...
from xnat_requests.sessions import create_session
from xnat_requests.format_paths_dict import create_paths_dict_for_subj_asr
    
    
def download_subj_asrs(url, proj_id, subj_label, session=None, 
    ...
    *******
    ...
    if export_root_dir == '':
        export_root_dir = os.path.join(Path.home(), "Downloads", 
                                       "xnat_downloads")
    ...
    for result in request.json()['ResultSet']['Result']:
        ...
        export_dir = os.path.join(export_root_dir, 'projects', proj_id, 'subjects',
                                 subj_label, 'resources', res_id, 'files',
                                 name)
        ...
        paths_dict, keys = create_paths_dict_for_subj_asr(proj_id, subj_label, 
                                                          res_id, name, 
                                                          paths_dict)
        
        paths_dict['projects'][proj_id]['subjects'][subj_label]['resources']\
        [res_id]['files'][name].update({'name' : name,
                                        'res_id' : res_id,
...
def upload_subj_asr(subj_asr_fpath, url, proj_id, subj_label, 
                    content_label='SRO-DRO', session=None, 
                    username=None, password=None):
    """
    ...
    *******
    ...
    subj_asr_fname = os.path.split(subj_asr_fpath)[1]
    
    
    """ Upload the resource: """
    with open(subj_asr_fpath, 'rb') as file:
        uri = f'{url}/data/projects/{proj_id}/subjects/{subj_label}/'\
              + f'resources/DICOM/files/{subj_asr_fname}'
        
        request = session.put(f'{uri}?inbody=true&file_format=DICOM'
