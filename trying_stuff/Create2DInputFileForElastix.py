...
"""
Function:
    Create2DInputFileForElastix()
    
Purpose:
    Create a text file containing 2D points for Elastix's Transformix to deform.
    ...
    called GetContourPoints2D(), that uses tags in the DICOMs (Dicoms) and the 
    ...
    [
     ...
     ...
     [
      [point1_x_contourN, point1_y_contourN],
      [point2_x_contourN, point2_y_contourN],
      ...
      [pointM_x_contourN, pointM_y_contourN]
      ], 
     [],
     ]
...
def Create2DInputFileForElastix(DicomDir, RoiFpath):
    # Import packages:
    import pydicom
    from GetDicoms import GetDicoms
    #from GetContourPoints3D import GetContourPoints3D
    import importlib
    import Get2DContourPointsFromDicoms
    importlib.reload(Get2DContourPointsFromDicoms)
    ...
    ContourPts = Get2DContourPointsFromDicoms(DicomDir, RoiFpath, 
                                              Flattened=False)
    ...
    for ArrayOfPts in ContourPts:
        if ArrayOfPts:
            # Loop through each array of 2D points within each ArrayOfPts and 
