...
# ### It seems that slice 17 and 20 from fixed and moving look reasonably close in scan depth.
...
FixedFpaths = GetDicomFpaths(DataDict['Source']['DicomDir'], SortMethod='slices', Debug=False)
MovingFpaths = GetDicomFpaths(DataDict['Target']['DicomDir'], SortMethod='slices', Debug=False)


# In[7]:


# Read in the 17th and 20th slices for fixed and moving:
Fixed2D = ants.image_read(FixedFpaths[17])
Moving2D = ants.image_read(MovingFpaths[20])
...
# Fixed3D = ants.image_read(FixedDir)

# In[21]:


# Register Moving2D to Fixed2D:

reg = ants.registration(Fixed2D, Moving2D, 'Affine')

Reg2D = reg['warpedmovout']
...
#plt.imshow(Fixed2D[:,:,0], cmap=plt.cm.Greys_r);
# Need to rotate by 90 deg 3x and flip lr to get familiar orientation:
plt.imshow(np.fliplr(np.rot90(Fixed2D[:,:,0], 3)), cmap=plt.cm.Greys_r);
plt.title('Fixed2D')
plt.axis('off')

# draw the moving image in the second subplot
plt.subplot(1, 3, 2, aspect='equal')
#plt.imshow(Moving2D[:,:,0], cmap=plt.cm.Greys_r);
# Need to rotate by 90 deg 3x and flip lr to get familiar orientation:
plt.imshow(np.fliplr(np.rot90(Moving2D[:,:,0], 3)), cmap=plt.cm.Greys_r);
plt.title('Moving2D')
plt.axis('off')

# draw the registered image in the third subplot
plt.subplot(1, 3, 3, aspect='equal')
#plt.imshow(Reg2D[:,:,0], cmap=plt.cm.Greys_r);
# Need to rotate by 90 deg 3x and flip lr to get familiar orientation:
plt.imshow(np.fliplr(np.rot90(Reg2D[:,:,0], 3)), cmap=plt.cm.Greys_r);
plt.title('Reg2D')
plt.axis('off')


# ### I really need to be working in 3D. 
...
FixedIm = ants.dicom_read(FixedDir)
MovingIm = ants.dicom_read(MovingDir)
...
# ### Took only 6 s to register the 3D stacks.  By comparison, SimpleElastix took 23 s!!
